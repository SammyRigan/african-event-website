    ...
    const fetchCategories = async () => {
      try {
        ...
        if (categoriesData.filter(cat => cat.isActive).length > 0) {
          setCategory(categoriesData.filter(cat => cat.isActive)[0].key);
        }
      } catch (error) {
          ...
          <CardContent className="space-y-6">
            ...
            {selectedFiles.length > 0 && !uploadComplete && (
              <div className="flex gap-4 pt-4 border-t">
                <Button type="button" variant="outline" className="flex-1 w-full" disabled={uploading} onClick={() => router.back()}>
                  Cancel
                </Button>
                <Button
