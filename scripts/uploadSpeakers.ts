...
export async function uploadSpeakers() {
  console.log('Starting speaker upload...');
  
  for (const speaker of speakers) {
    try {
      const result = await addSpeaker(speaker);
      console.log(`✅ Uploaded: ${speaker.name} (ID: ${result.id})`);
    } catch (error) {
      console.error(`❌ Failed to upload ${speaker.name}:`, error);
    }
  }
  
  console.log('Speaker upload complete!');
}

// Run if called directly
if (require.main === module) {
  uploadSpeakers().catch(console.error);
}
