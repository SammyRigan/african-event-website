...
import { updateSpeaker, uploadSpeakerImage } from '@/lib/firebaseService';
...
export default function EditSpeakerClient({ speakerId }: EditSpeakerClientProps) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setFormData({
            ...
            achievementsFr: data.achievementsFr?.join('\n') || '',
            education: data.education?.join('\n') || '',
            educationFr: data.educationFr?.join('\n') || '',
            expertise: data.expertise?.join('\n') || '',
            expertiseFr: data.expertiseFr?.join('\n') || '',
            impact: data.impact?.join('\n') || '',
            impactFr: data.impactFr?.join('\n') || '',
            initiatives: data.initiatives?.join('\n') || '',
            initiativesFr: data.initiativesFr?.join('\n') || '',
            experience: data.experience?.join('\n') || '',
            experienceFr: data.experienceFr?.join('\n') || '',
            order: data.order || 1,
    ...
    fetchSpeaker();
  }, [speakerId, router, isAuthenticated]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        setUploading(true);
        imageUrl = await uploadSpeakerImage(imageFile, formData.name);
        setUploading(false);
      }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-12 h-12 animate-spin text-[#E19D2B] mx-auto mb-4" />
          <p className="text-gray-600">Loading speaker data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">Edit Speaker</CardTitle>
            <p className="text-sm text-gray-600 mt-2">
              Update the speaker details below. All fields with * are required.
            </p>
          </CardHeader>
          <CardContent>
                  ...
                  <div className="mt-2 space-y-4">
                    ...
                    {!imagePreview && (
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
                        ...
                        >
                          <span className="text-[#E19D2B] font-semibold">Upload new image</span>
                          <br />
